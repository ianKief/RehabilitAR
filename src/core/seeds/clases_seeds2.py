...
from src.core.clases.clases import Clase, ClaseBloque
...
class ClaseSeeder2:
    ...
    def run(self):
        print("Insertando clases de prueba (versión 2, con bloques)...")
        ...
        for i in range(8):
            ...
            for f in fechas_a_crear:
                clase = Clase(
                    nombre=nombre,
                    especialidad=especialidad_nombre_enum.value,
                    duracion=duracion,
                    capacidad_maxima=capacidad_maxima,
                    descripcion=f"Clase de prueba ({tipo}). Enfoque: {especialidad_nombre_enum.value.lower()}. Espacio controlado en Sala {sala_asignada.numero_puerta}.",
                    suspendida=False,
                    fecha_clase=f,
                    horario=horario,
                    aprobada=True,
